  ...
  1. """
    ...
    if response.result != None:
        ...
        name_match = re.search(r'<name>(.*?)</name>',
                               response.result, re.DOTALL)
        char_style_match = re.search(
            r'<char-style>(.*?)</char-style>', response.result, re.DOTALL)
        dialogue_matches = re.findall(
            r'<dialogue>(.*?)</dialogue>', response.result, re.DOTALL)
        ...
        char_style = char_style_match.group(
            1).strip() if char_style_match else None
